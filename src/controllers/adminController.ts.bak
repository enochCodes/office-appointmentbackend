...
export const adminController = {
  async createUser(req: Request, res: Response) {
    ...
    try {
      const userCreationPayload: AuthUserCreationData = { name, email, phone, password, role };
  ...
  async createDirector(req: Request, res: Response) {
    ...
    try {
      const newDirector = await adminService.createDirectorProfile(userId, directorData);
      return res.status(201).json({ message: 'Director profile created successfully.', director: newDirector });
    } catch (error: any) {
      console.error('Admin createDirector error:', error.message);
      if (error.message.startsWith('Validation Error:') || error.message.startsWith('Conflict Error:')) {
        return res.status(error.message.startsWith('Conflict Error:') ? 409 : 400).json({ message: error.message });
      }
      return res.status(500).json({ message: error.message || 'Failed to create director profile.' });
    }
  },
  ...
  async createAssistant(req: Request, res: Response) {
    ...
    try {
      const newAssistant = await adminService.createAssistantProfile(userId, directorId /*, assistantData */);
      return res.status(201).json({ message: 'Assistant profile created successfully.', assistant: newAssistant });
    } catch (error: any) {
      console.error('Admin createAssistant error:', error.message);
       if (error.message.startsWith('Validation Error:') || error.message.startsWith('Conflict Error:')) {
        return res.status(error.message.startsWith('Conflict Error:') ? 409 : 400).json({ message: error.message });
      }
      return res.status(500).json({ message: error.message || 'Failed to create assistant profile.' });
    }
  },
};
